...
function defineGour_net(input_nc, output_nc, ngf)
	local netG = nil
	 ...
	 local e0_1 = e0   - nn.LeakyReLU(0.2, true) - nn.SpatialConvolution(ngf, ngf , 3, 3, 1, 1, 1, 1) - nn.SpatialBatchNormalization(ngf) 
   	 ...
   	 local e6 = e5 - nn.LeakyReLU(0.2, true) - nn.SpatialConvolution(ngf * 16, ngf * 16, 4, 4, 2, 2, 1, 1) - nn.SpatialBatchNormalization(ngf * 16)

   	 -- input is (ngf * 8) x 4 x 4

   	 local e7 = e6 - nn.LeakyReLU(0.2, true) - nn.SpatialConvolution(ngf * 16, ngf * 16, 4, 4, 2, 2, 1, 1) - nn.SpatialBatchNormalization(ngf * 16)

   	 -- input is (ngf * 8) x 2 x 2

   	 local e8 = e7 - nn.LeakyReLU(0.2, true) - nn.SpatialConvolution(ngf * 16, ngf * 16, 4, 4, 2, 2, 1, 1) - nn.SpatialBatchNormalization(ngf * 16)

   	 -- input is (ngf * 8) x 1 x 1



   	 local d1_ = e8 - nn.ReLU(true) - nn.SpatialFullConvolution(ngf * 16, ngf * 16, 4, 4, 2, 2, 1, 1) - nn.SpatialBatchNormalization(ngf * 16) 

   	 -- input is (ngf * 8) x 2 x 2

   	 local d1 = {d1_,e7} - nn.CAddTable()

   	 local d2_ = d1 - nn.ReLU(true) - nn.SpatialFullConvolution(ngf * 16 , ngf * 16, 4, 4, 2, 2, 1, 1) - nn.SpatialBatchNormalization(ngf * 16)

   	 -- input is (ngf * 8) x 4 x 4

   	 local d2 = {d2_,e6} - nn.CAddTable()

   	 local d3_ = d2 - nn.ReLU(true) - nn.SpatialFullConvolution(ngf * 16 , ngf * 16, 4, 4, 2, 2, 1, 1) - nn.SpatialBatchNormalization(ngf * 16)

   	 -- input is (ngf * 8) x 8 x 8

   	 local d3 = {d3_,e5} - nn.CAddTable()

   	 local d4_ = d3 - nn.ReLU(true) - nn.SpatialFullConvolution(ngf * 16 , ngf * 8, 4, 4, 2, 2, 1, 1) - nn.SpatialBatchNormalization(ngf * 8)

   	 -- input is (ngf * 8) x 16 x 16

   	 local d4 = {d4_,e4} - nn.CAddTable()

   	 local d5_ = d4 - nn.ReLU(true) - nn.SpatialFullConvolution(ngf * 8 , ngf * 4, 4, 4, 2, 2, 1, 1) - nn.SpatialBatchNormalization(ngf * 4)

   	 -- input is (ngf * 4) x 32 x 32

   	 local d5 = {d5_,e3} - nn.CAddTable()

   	 local d6_ = d5 - nn.ReLU(true) - nn.SpatialFullConvolution(ngf * 4 , ngf * 2, 4, 4, 2, 2, 1, 1) - nn.SpatialBatchNormalization(ngf * 2)

   	 -- input is (ngf * 2) x 64 x 64

   	 local d6 = {d6_,e2} - nn.CAddTable()

   	 local d7_ = d6 - nn.ReLU(true) - nn.SpatialFullConvolution(ngf * 2 , ngf, 4, 4, 2, 2, 1, 1) - nn.SpatialBatchNormalization(ngf)

   	 -- input is (ngf) x128 x 128

   	 local d7 = {d7_,e1} - nn.CAddTable()

   	 local d8 = d7 - nn.ReLU(true) - nn.SpatialFullConvolution(ngf , ngf, 4, 4, 2, 2, 1, 1) - nn.SpatialBatchNormalization(ngf)
